...
bool InverterModel::IsInverterRegisterMap(void)
{
	...
	if ( !((I11X_ID.res == 111) || (I11X_ID.res == 112) || (I11X_ID.res = 113)) ) {
		ErrorMessage = std::string("Invalid ID of Inverter Model block (") + std::to_string(I11X_ID.res) + ")";
		return false;
	}
	...
	if ( I11X_L.res != 60 ) {
		ErrorMessage = std::string("Invalid length of Inverter Model block (") + std::to_string(I11X_L.res) + ")";
		return false;
	}
