...
bool FroniusInverter::SetStateCodePrimo(StateCode_t &state)
{
	switch (state.St)
	{
	case 0:
	...
	case 245:
		...
		break;
	case 247:
		state.StStr = "Arc Fault Cicruit Interrupter (AFCI) current sensor error";
		break;
	case 249:
		state.StStr = "Arc Fault Cicruit Interrupter (AFCI) detected unplausibel measurement values";
		break;
	case 301:
	...
	case 766:
		...
		break;
	default:
		ErrorMessage = "Invalid Fronius state code (" + std::to_string(state.St) + ")";
		return false;
	}
...
bool FroniusInverter::SetStateCodeSymo(StateCode_t &state)
{
	return true;
}

bool FroniusInverter::SetStateCodeGalvo(StateCode_t &state)
{
	return true;
}

bool FroniusInverter::SetStateCodeIgplus(StateCode_t &state)
{
	return true;
}
...
bool FroniusInverter::GetStateCode(StateCode_t &state)
{
	...
	std::string model;
	if (!GetDeviceModel(model)) {
		return false;
	}

	if (model.find("Primo") != std::string::npos) {
		if (!SetStateCodePrimo(state)) {
			return false;
		}
	} else if (model.find("Symo") != std::string::npos) {
		if (!SetStateCodeSymo(state)) {
			return false;
		}
	} else if (model.find("Galvo") != std::string::npos) {
		if (!SetStateCodeGalvo(state)) {
			return false;
		}
	} else if (model.find("IGPlus") != std::string::npos) {
		if (!SetStateCodeIgplus(state)) {
			return false;
		}
	} else {
		ErrorMessage = "Invalid Fronius model:" + model;
		return false;
	}
