...
bool ImmediateModel::IsImmediateRegisterMap(void) {
  ...
  if (!(I123_ID.res == 123)) {
    ErrorMessage = std::string("Invalid ID of Immediate Controls Model block (")
        + std::to_string(I123_ID.res) + ")";
    return false;
  }
  ...
  if (I123_L.res != 24) {
    ErrorMessage = std::string(
        "Invalid length of Immediate Controls Model block (")
        + std::to_string(I123_L.res) + ")";
    return false;
  }
