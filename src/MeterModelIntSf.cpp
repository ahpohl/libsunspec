...
bool MeterModel::IsMeterRegisterMap(void)
{
	...
	if ( !((M20X_ID.res == 201) || (M20X_ID.res == 202) || (M20X_ID.res = 203)) ) {
		ErrorMessage = std::string("Invalid ID of Meter Model block (") + std::to_string(M20X_ID.res) + ")";
		return false;
	}
	...
	if ( M20X_L.res != 105 ) {
		ErrorMessage = std::string("Invalid length of Meter Model block (") + std::to_string(M20X_L.res) + ")";
		return false;
	}
...
bool MeterModel::GetAcVoltagePhaseToPhase(double &res, const std::string &ph_pair)
{
	if (ph_pair == "AB") {
...
bool MeterModel::GetAcEnergyExport(double &res)
{
	...
	res = static_cast<double>(M20X_TotWhExp.res) * pow(10, M20X_TotWh_SF.res) * 0.001f;
...
bool MeterModel::GetAcEnergyImport(double &res)
{
	...
	res = static_cast<double>(M20X_TotWhImp.res) * pow(10, M20X_TotWh_SF.res) * 0.001f;
