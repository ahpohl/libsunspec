...
bool MeterModel::IsMeterRegisterMap(void)
{
	...
	if ( !((M21X_ID.res == 211) || (M21X_ID.res == 212) || (M21X_ID.res = 213)) ) {
		ErrorMessage = std::string("Invalid ID of Meter Model block (") + std::to_string(M21X_ID.res) + ")";
		return false;
	}
	...
	if ( M21X_L.res != 65 ) {
		ErrorMessage = std::string("Invalid length of Meter Model block (") + std::to_string(M21X_L.res) + ")";
		return false;
	}
